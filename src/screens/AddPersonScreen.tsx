...
export default function AddPersonScreen() {
  ...
  const handleSave = () => {
    // Logic to save the person will go here
    navigation.goBack();
  };
      ...
      >
              ...
              <View className="flex-row flex-wrap gap-2">
                {['Family', 'Friend', 'Work', 'Other'].map((type) => (
                  <TouchableOpacity 
                    ...
                    className={`px-6 py-3 rounded-full border ${relationship === type ? 'bg-brand-green border-brand-green' : 'bg-brand-muted border-gray-800'}`}
                  >
        ...
        <View className="pb-8">
          <TouchableOpacity 
            ...
            disabled={!name || !phone}
            activeOpacity={0.7}
            className={`h-16 rounded-2xl items-center justify-center ${(!name || !phone) ? 'bg-gray-800 opacity-50' : 'bg-brand-green shadow-xl shadow-brand-green/20'}`}
          >
